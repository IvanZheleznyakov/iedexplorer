
//
// This file was generated by the BinaryNotes compiler.
// See http://bnotes.sourceforge.net 
// Any modifications to this file will be lost upon recompilation of the source ASN.1. 
//

using System;
using org.bn.attributes;
using org.bn.attributes.constraints;
using org.bn.coders;
using org.bn.types;
using org.bn;

namespace MMS_ASN1_Model {


    [ASN1PreparedElement]
    [ASN1Sequence ( Name = "Confirmed_RequestPDU", IsSet = false  )]
    public class Confirmed_RequestPDU : IASN1PreparedElement {
                    
	private Unsigned32 invokeID_ ;
	
        [ASN1Element ( Name = "invokeID", IsOptional =  false , HasTag =  false  , HasDefaultValue =  false )  ]
    
        public Unsigned32 InvokeID
        {
            get { return invokeID_; }
            set { invokeID_ = value;  }
        }
        
                
          
	private System.Collections.Generic.ICollection<Modifier> listOfModifiers_ ;
	
        private bool  listOfModifiers_present = false ;
	
[ASN1SequenceOf( Name = "listOfModifiers", IsSetOf = false  )]

    
        [ASN1Element ( Name = "listOfModifiers", IsOptional =  true , HasTag =  false  , HasDefaultValue =  false )  ]
    
        public System.Collections.Generic.ICollection<Modifier> ListOfModifiers
        {
            get { return listOfModifiers_; }
            set { listOfModifiers_ = value; listOfModifiers_present = true;  }
        }
        
                
          
	private ConfirmedServiceRequest service_ ;
	
        [ASN1Element ( Name = "service", IsOptional =  false , HasTag =  false  , HasDefaultValue =  false )  ]
    
        public ConfirmedServiceRequest Service
        {
            get { return service_; }
            set { service_ = value;  }
        }
        
                
          
	private Request_Detail service_ext_ ;
	
        private bool  service_ext_present = false ;
	
        [ASN1Element ( Name = "service-ext", IsOptional =  true , HasTag =  true, Tag = 79 , HasDefaultValue =  false )  ]
    
        public Request_Detail Service_ext
        {
            get { return service_ext_; }
            set { service_ext_ = value; service_ext_present = true;  }
        }
        
                
  
        public bool isListOfModifiersPresent () {
            return this.listOfModifiers_present == true;
        }
        
        public bool isService_extPresent () {
            return this.service_ext_present == true;
        }
        

            public void initWithDefaults() {
            	
            }

        //  STATICHERE
            private static IASN1PreparedElementData preparedData = CoderFactory.getInstance().newPreparedElementData(typeof(Confirmed_RequestPDU));
            public IASN1PreparedElementData PreparedData {
            	get { return preparedData; }
            }

            
    }
            
}
